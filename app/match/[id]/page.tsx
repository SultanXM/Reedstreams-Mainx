      ...
      <div className="match-page-container">
        <div className="top-nav">
           <div className="nav-back-wrapper">
              <div className="nav-back-icon"><ArrowLeft size={16} /></div>
              <div style={{fontSize:'20px', fontWeight:900, letterSpacing:'-1px', fontFamily:'sans-serif'}}>
                <span style={{color:'#fff'}}>REED</span><span style={{color:'#8db902'}}>STREAMS</span>
              </div>
           </div>
        </div>
...
function MatchPageContent() {
  ...
  return (
    <>
             ...
             {!feedbackSent ? (
               <form onSubmit={handleFeedbackSubmit}>
                 <div style={{display:'flex', alignItems:'center', gap:'10px', marginBottom:'20px'}}>
                    <AlertOctagon size={24} color="#8db902" />
                    {/* 🔥 TRANSLATED REPORT HEADER */}
                    <h2 style={{color:'#fff', fontSize:'18px', fontWeight:'800', margin:0}}>{t.report || "Report Issue"}</h2>
                 </div>
                 ...
                 >
                    <option>{t.stream_lag || "Stream Lag"}</option>
                    <option>{t.audio_sync || "Audio Sync"}</option>
                    <option>{t.stream_down || "Stream Down"}</option>
                    <option>{t.other_issue || "Other"}</option>
                 </select>
                 ...
                 >
                   <Send size={14} /> {t.submit_report || "Submit"}
                 </button>
               </form>
             ) : (
               <div style={{textAlign:'center', padding:'30px 0'}}>
                 <CheckCircle size={48} color="#8db902" style={{margin:'0 auto 15px auto'}} />
                 <h3 style={{color:'#fff', margin:'0 0 5px 0'}}>{t.report_sent || "Report Sent!"}</h3>
                 <p style={{color:'#666', fontSize:'12px'}}>{t.thank_you_report || "Thanks"}</p>
               </div>
             )}
          </div>
        </div>
      ...
      <div className="match-page-container" style={{ background: cinemaMode ? '#000' : '#050505' }}>
        
        {/* 1. TOP NAV BAR */}
        {!cinemaMode && (
          <div className="top-nav">
            <div className="nav-back-wrapper" onClick={() => router.back()}>
              <div className="nav-back-icon">
                <ArrowLeft size={18} />
              </div>

              <div style={{fontSize:'22px', fontWeight:900, letterSpacing:'-1px', fontFamily:'sans-serif'}}>
                <span style={{color:'#fff'}}>REED</span>
                <span style={{color:'#8db902'}}>STREAMS</span>
              </div>
            </div>
            ...
            <div style={{display:'flex', gap:'10px'}}>
               ...
               >
                 <AlertOctagon size={14} color="#f00" /> <span style={{display: 'none', md: 'inline'}}>{t.report || "Report"}</span>
               </button>
            </div>
          </div>
        )}
        ...
        <div className={`match-grid ${cinemaMode ? 'layout-cinema' : 'layout-standard'}`}>
                ...
                <div style={{display:'flex', flexDirection:'column', alignItems:'flex-end', gap:'10px', width: '100%', flex: '1', maxWidth: '500px'}}>
                   <div className="controls-group">
                      ...
                      <button 
                        ...
                        style={{
                          background: '#111', border: '1px solid #222', color: cinemaMode ? '#8db902' : '#ccc',
                      ...
                      >
                        {cinemaMode ? <Minimize size={14}/> : <Tv size={14}/>}
                      ...
                      {!cinemaMode && (
                         <div style={{display:'flex', gap:'8px', flex:1, justifyContent:'center'}}>
                            ...
                            <button onClick={() => openShare(shareLinks.whatsapp)} style={{background:'#000', border:'1px solid #222', padding:'10px', borderRadius:'6px', cursor:'pointer'}}>
          ...
          {!cinemaMode && (
            <div className="chat-panel">
                ...
                {/* CHAT HEADER */}
                <div style={{padding:'15px', borderBottom:'1px solid #222', display:'flex', justifyContent:'space-between', alignItems:'center', background:'#0f0f0f'}}>
                    {/* 🔥 TRANSLATED HEADER */}
                    <span style={{color:'#fff', fontWeight:'bold', fontSize:'13px'}}>{t.live_chat || "Live Chat"}</span>
                    <div style={{display:'flex', alignItems:'center', gap:'6px', fontSize:'11px', color:'#888'}}>
                      <div style={{width:'6px', height:'6px', background:'#8db902', borderRadius:'50%'}}></div> 
                      {/* 🔥 TRANSLATED ONLINE STATUS */}
                      <span style={{color:'#8db902', fontWeight:'bold'}}>{t.online || "Online"}</span>
                    </div>
                </div>
                ...
                <div style={{flex:1, position:'relative', background:'#050505'}}>
                  ...
                  {/* 🔒 "GOOD BOY" BLUR OVERLAY */}
                  {!chatAgreed && (
                    <div style={{
                      position:'absolute', top:0, left:0, width:'100%', height:'100%', 
                      background:'rgba(5,5,5,0.95)', backdropFilter:'blur(4px)', zIndex:10,
                      ...
                      <button 
                        ...
                        style={{
                          background:'#8db902', color:'#000', border:'none', padding:'10px 30px', 
