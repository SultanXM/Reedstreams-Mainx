...
const ADMIN_KEY: &str = "obsessed_boss_2026"; // Hardcoded or use env in prod
...
async fn main() {
    ...
    // Background task to prune expired views
    ...
    println!("Rust Backend (Views + Chat) running on {}", addr);
...
async fn get_messages(
    ...
    let filtered: Vec<ChatMessage> = messages.iter()
        .filter(|m| m.match_id == query.match_id)
        .cloned()
        .collect();
    Json(filtered)
}
...
async fn delete_message(
    ...
    if payload.admin_key.to_lowercase() != ADMIN_KEY.to_lowercase() {
