...
export default function MatchPlayer({ matchId }: { matchId: string }) {
        ...
        async function init() {
            try {
                ...
                if (allStreams.length === 0) {
                    ...
                    let best = null;
                    best = allStreams.find(s => s.sourceIdentifier.toLowerCase().includes("bravo") && s.streamNo === 1);
                    if (!best) best = allStreams.find(s => s.sourceIdentifier.toLowerCase().includes("bravo") && s.streamNo === 2);
                    if (!best) best = allStreams.find(s => s.sourceIdentifier.toLowerCase().includes("golf") && s.streamNo === 1);
                    if (!best) best = allStreams.find(s => s.sourceIdentifier.toLowerCase() === "delta" && s.streamNo === 1);
                    if (!best) best = allStreams.find(s => s.sourceIdentifier.toLowerCase() === "admin" && s.streamNo === 1);
    ...
    return (
        <div className="player-wrapper">
            ...
            {/* 🔥 ALWAYS SHOW SELECTOR IF WE HAVE AT LEAST 1 STREAM */}
            {streams.length > 0 && (
