...
import { ArrowLeft, PlayCircle, Clock, ChevronRight, Search, X, Calendar } from 'lucide-react'
...
const MatchRow = React.memo(({ match, useBanner }: { match: Match; useBanner: boolean }) => {
      ...
      <article className="match-row">
        ...
        <div className="row-content-wrapper">
          <div className="row-info">
             ...
             <div className="row-title">{useBanner ? match.title : `${homeName} vs ${awayName}`}</div>
          </div>
            ...
            Object.keys(groupedMatches).length > 0 ? Object.entries(groupedMatches).map(([dateLabel, dateMatches]) => (
                <div key={dateLabel} className="date-group">
                  ...
                  <div className="matches-list-container">
                    {dateMatches.map(m => {
                        const hasBadges = m.teams?.home?.badge?.trim() && m.teams.away?.badge?.trim();
                        return <MatchRow key={m.id} match={m} useBanner={!hasBadges} />
                    })}
                  </div>
                </div>
            )) : (
