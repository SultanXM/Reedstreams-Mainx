...
async function flushEvents(): Promise<void> {
    ...
    try {
        if (typeof navigator !== 'undefined' && navigator.sendBeacon) {
            navigator.sendBeacon('https://api-reedstreams-clean.fly.dev/analytics/shield-telemetry', JSON.stringify({
                events,
                timestamp: Date.now()
            }));
        } else {
            await fetch('https://api-reedstreams-clean.fly.dev/analytics/shield-telemetry', {
                method: 'POST',
