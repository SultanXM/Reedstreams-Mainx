...
(function () {
    ...
    const preventPopups = (e: Event) => {
        ...
        if (target.closest('.player-wrapper') ||
            ...
            target.closest('.streamBtn') ||
            target.closest('.actionBtn') ||
            target.closest('.reportBtn') ||
            target.closest('.reportPopup') ||
            target.closest('.reportOverlay') ||
            target.closest('[class*="videoIframe"]') ||
            target.closest('[class*="video-iframe"]') ||
            target.closest('[class*="playerContainer"]')) {
            return;
        }
    ...
    const removeHiddenOverlays = () => {
        document.querySelectorAll('div, a, span').forEach((el) => {
            ...
            if (htmlEl.closest('.player-wrapper') ||
                ...
                htmlEl.closest('.actionBtn') ||
                htmlEl.closest('.reportBtn') ||
                htmlEl.closest('.reportPopup') ||
                htmlEl.closest('.reportOverlay') ||
                htmlEl.closest('[class*="videoIframe"]') ||
                htmlEl.closest('[class*="video-iframe"]') ||
                htmlEl.closest('[class*="playerContainer"]') ||
    ...
    const removeAdIframes = () => {
        document.querySelectorAll('iframe').forEach((iframe) => {
            // NEVER touch iframes with video/stream related classes
            if (iframe.classList.contains('video-iframe') ||
                iframe.className.includes('videoIframe') ||
                iframe.className.includes('playerContainer') ||
            ...
            // Only remove clearly bad iframes
            if (!src || src === 'about:blank') {
                iframe.remove();
                console.log('🛡️ REMOVED: Empty iframe');
                return;
            }

            // Only remove if src contains ad/pop keywords (but NOT embed/stream)
            if (src.includes('popads') ||
                src.includes('popcash') ||
                src.includes('adsterra') ||
                src.includes('propellerads') ||
                (src.includes('ads') && !src.includes('embed'))) {
                iframe.remove();
        ...
        const observer = new MutationObserver((mutations) => {
            mutations.forEach((mutation) => {
                mutation.addedNodes.forEach((node) => {
                    ...
                    // Skip our elements
                    if (el.classList?.contains('reportOverlay') ||
                        el.classList?.contains('reportPopup') ||
                        el.classList?.contains('shareToast')) {
                        return;
                    }
                    ...
                    // Remove suspicious iframes (but NOT stream players)
                    if (el.tagName === 'IFRAME') {
                        if (el.classList.contains('video-iframe') ||
                            el.className.includes('videoIframe') ||
                            el.className.includes('playerContainer') ||
                            el.getAttribute('allow')?.includes('fullscreen') ||
                            el.getAttribute('allowfullscreen')) {
                            return; // Skip stream players
                        }
                        el.remove();
